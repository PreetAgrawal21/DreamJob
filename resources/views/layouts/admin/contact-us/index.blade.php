			...
			<div class="row">
				<div class="col-12">
					<div class="card">
						...
						<!-- /.card-header -->
						<div class="card-body">
							<table id="contactus-yajra" class="table table-bordered table-striped">
								<thead>
									<tr>
										<th>{{ __('No') }}</th>
										<th>{{ __('Name') }}</th>
...
@section('js')
	<script type="text/javascript">
		$(function () {
			...
			$('#contactus-yajra thead tr:eq(1) th:eq(1)').each( function (){
					var title = $(this).text();
					$(this).html( '<input type="text" class="form-control" placeholder="Search '+title+'" />' );
				}
			);

			var table = $('#contactus-yajra').DataTable({
				responsive		: true,
				processing		: false,
				serverSide		: true,
				deferRender		: true,
				...
				ajax: {
					url	: "{{ route('contact-us.getcontact-us') }}",
					type: "POST",
					 data:	{
					 	_token: "{{ csrf_token() }}",
					 },
